...
pipeline {
  ...
  stages {
    ...
    stage('Cleanup Jenkins agent VM') {
      steps {
        sh ('automation-git/docs/pipelines/stage-03/scripts/cleanup-jenkins-agent-vm.sh')
      }
    }
    ...
    stage('Provision agent VM') {
      steps {
        sh ('''
            ...
            for i in $(seq 20)
            do
                SSHPASS=linux automation-git/docs/pipelines/stage-03/scripts/sshpass.sh ssh-copy-id root@${AGENT_IP}
                if (( $? == 0 )); then
