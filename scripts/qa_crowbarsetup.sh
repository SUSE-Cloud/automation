...
# by setting --dns-nameserver for subnet, docker instance gets this as DNS info
# (otherwise it would use /etc/resolv.conf from its host)
function adapt_dns_for_docker()
{
    local partialns=`neutron subnet-show fixed | grep network_id | sed 's/\s//g' | cut -d "|" -f 3`
    local dns_server=`grep dns-server /var/lib/neutron/dhcp/$partialns/opts | cut -d ":" -f 3 | cut -d "," -f 2`
