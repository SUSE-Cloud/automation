    ...
    case "$proposal" in
        ...
        ceilometer)
            ...
            if [ -n "$want_sles12" ] && iscloudver 5plus ; then
                ...
                local ceilometeragents=`crowbar ceilometer proposal show default | ruby -e "require 'rubygems';require 'json';puts JSON.parse(STDIN.read)['deployment']['ceilometer']['elements']['ceilometer-agent']" | sed '/'"${sles12node}"'/d'`
