...
case "$cloudsource" in
  ...
  ;;
  openstackhavana)
	$zypper ar -G -f http://download.opensuse.org/repositories/Cloud:/OpenStack:/Havana/$REPO/Cloud:OpenStack:Havana
	if test -n "$OSHEAD" ; then
...
case "$cloudsource" in
  develcloud1*|develcloud2.0|openstackessex|openstackfolsom)
        cn=""
        tempest=""
  ;;
  *)
