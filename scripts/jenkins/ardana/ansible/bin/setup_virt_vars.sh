...
if [ -n "$(grep "^ardana-$ardana_env\s\+ansible_host" inventory)" ]; then
  sed -i "s/^ardana-$ardana_env.*/ardana-$ardana_env      ansible_host=$DEPLOYER_IP/g" inventory
else
  sed -i "/^\[deployer_virt\]/a ardana-$ardana_env      ansible_host=$DEPLOYER_IP" inventory
fi
...
cat << EOF > host_vars/ardana-$ardana_env.yml
---
...
echo "controller_mgmt_ips:" >> host_vars/ardana-$ardana_env.yml
...
echo "compute_mgmt_ips:" >> host_vars/ardana-$ardana_env.yml
...
echo "dns_servers:" >> host_vars/ardana-$ardana_env.yml
openstack --os-cloud ${os_cloud} port list --network $NETWORK_MGMT_ID \
  ...
  while read line; do echo "  - $line" >> host_vars/ardana-$ardana_env.yml; done;

cat host_vars/ardana-$ardana_env.yml
